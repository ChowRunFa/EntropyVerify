...
def process_jsonl(input_files: List[str], output_file: str, top_percent: float = None, top_k: int = None):
    ...
    for input_file in input_files:
        try:
            ...
            print(f"Warning: Input file not found, skipped: {input_file}")

    if not all_data:
        print("Input file(s) are empty or not found. Processing finished.")
        return
...
# ==============================================================================
# Example: Demonstrate grouped scoring functionality
# ==============================================================================
if __name__ == "__main__":
    ...
    input_filenames = [
        os.path.join(DATA_DIR, 'with_entropy_gsm8k_20250724230200.jsonl'),
        os.path.join(DATA_DIR, 'with_entropy_gsm8k_20250724230053.jsonl'),
        os.path.join(DATA_DIR, 'with_entropy_gsm8k_20250724230021.jsonl')
    ]
    output_filename = os.path.join(DATA_DIR, "with_entropy_gsm8k_test_fill.jsonl")

    process_jsonl(input_filenames, output_filename, top_percent=0.2)